...
class StorageMotherboardAnalyzerTest {

    @Test
    void shouldReportIncompatibleNvmeStorageWithoutM2Slots() {
        CompatibilityRequest request = CompatibilityRequest.builder()
                ...
                .motherboard(ComponentFactory.motherboard("ASUS", "Prime", CpuSocket.AM5, RamType.DDR5, 128, 4, 6000, PcieVersion.PCIE_5, FormFactor.ATX, 4, 0, BigDecimal.valueOf(250)))
        ...
        new StorageMotherboardAnalyzer().analyze(request, builder);

        assertTrue(builder.errors().contains("Storage NVMe no es compatible porque la motherboard no tiene slots M.2 disponibles"));
    }
}